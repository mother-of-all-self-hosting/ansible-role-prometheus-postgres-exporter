...
#
# Tags look like `v<Postgres exporter version>-<release>`:
#
# - if defaults/main.yml points at a Postgres exporter version that has never
#   been released, the release counter restarts at 0 (`v0.20.1-0`)
# - otherwise the counter is incremented (`v0.20.1-1`), but only if something
...
# (`prometheus_postgres_exporter_container_image_tag`, for one) can be picked up
# instead.
version="$(sed -nE 's|^prometheus_postgres_exporter_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Postgres exporter version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.20.1`),
